                ...
                {project.isInstruction ? (
                  ...
                  <>
                    <div className="flex justify-between mb-2">
                      ...
                      <div className="flex flex-col items-center gap-1">
                        ...
                        <div className="flex gap-1">
                          ...
                          {savedByOthers.length > 0 && <span className="text-xs text-purple-400 flex items-center" title={`Also saved by: ${savedByOthers.map(s => s.savedBy).join(', ')}`}><Users size={12}/>{savedByOthers.length}</span>}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex gap-2">
                      ...
                      <button onClick={e => { e.stopPropagation(); toggleApplied(project); }} className={`px-3 py-2 rounded-lg ${isApplied ? 'bg-cyan-600 text-white' : 'bg-slate-700 hover:bg-slate-600'}`} title={isApplied ? 'Mark as not applied' : 'Mark as applied'}><Send size={16}/></button>
            ...
            {proposals[selectedProject.id] ? (
              <div className="space-y-4">
                ...
                <div className="flex gap-2 flex-wrap">
                  ...
                  <a href={selectedProject.link} target="_blank" rel="noopener noreferrer" className="flex-1 py-2 bg-indigo-600 hover:bg-indigo-500 rounded-lg text-center flex items-center justify-center gap-1">Apply on Upwork <ExternalLink size={14}/></a>
                </div>
              </div>
            ) : (
      ...
      {showSettings && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 z-50" onClick={() => setShowSettings(false)}>
          <div className="glass rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-5" onClick={e => e.stopPropagation()}>
            ...
            <button onClick={() => { setOfferings([...offerings, { name: 'New Service', keywords: ['keyword'], rateMin: 50, rateMax: 100, skills: ['Skill'] }]); setEditingOffering(offerings.length); }} className="w-full py-2 bg-slate-700 hover:bg-slate-600 rounded-lg flex items-center justify-center gap-2"><Plus size={18}/> Add Category</button>
          </div>
        </div>
      )}
    </div>
